...
// Types
import { ISourceMap, SOURCE_MAP_ALL_ATTRIBUTE_NAMES, SOURCE_MAP_REQUIRED_ATTRIBUTE_NAMES } from '../../src/types'
...
describe('SourceMap', () => {
	describe('instance related', () => {
		...
		test('getOriginalSourceLocation', async () => {
			// non existing location
			expect(await instance.getOriginalSourceLocation(1, 1)).toBeUndefined()

			// existing location
			expect(await instance.getOriginalSourceLocation(6, 0)).toEqual({
				column: 0,
				line: 1,
				name: null,
				source: '../src/index.ts'
	...
	describe('deserialization', () => {
		describe('full', () => {
			...
			const sourceMappingURLPrefix = '//# sourceMappingURL=data:application/json;base64,'
			...
			test('base64 deserialization', () => {
				...
				expect(sourceMapFromBase64.toBase64String()).toEqual(expectedBase64String)
			})
		...
		describe('minimal', () => {
			...
			test('all fields are present', () => {
				expect(Object.keys(expected)).toEqual(SOURCE_MAP_REQUIRED_ATTRIBUTE_NAMES)
			})
	...
	describe('loading from File', () => {
		it('extracts the inline source map', async () => {
			...
			expect(await sourceMap?.numberOfLinesInCompiledFile()).toEqual(28)
		})

		it('extracts the extern source map', async () => {
			...
			const sourceRedirect = fs
				.readFileSync((sourceMapRedirect as SourceMapRedirect).sourceMapLocation.toPlatformString())
				.toString()
