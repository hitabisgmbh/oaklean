...
const EXAMPLE_PROJECT_REPORT = ProjectReport.consumeFromBuffer(
	EXAMPLE_PROJECT_REPORT_BUFFER
).instance
...
describe('RegistryHelper', () => {
	describe('uploadToRegistry', () => {
		...
		test('default behaviour', async () => {
			const exampleAuthKey =
				'12345678-0000-0000-0000-000000000000' as UUID_string

			const axiosPostMock = jest.fn().mockResolvedValue({ data: 'data' })
			const axiosPostSpy = jest
				.spyOn(axios, 'post')
				.mockImplementation(axiosPostMock)
			jest
				.spyOn(AuthenticationHelper, 'getAuthentication')
				.mockResolvedValue(exampleAuthKey)

			const expectedCompressedBuffer = await BufferHelper.compressBuffer(
				EXAMPLE_PROJECT_REPORT.toBuffer()
			)
			...
			expect(
				Array.from((axiosPostMock.mock.calls[0][1] as FormData).keys()).length
			).toBe(2)
