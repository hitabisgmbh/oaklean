	...
	static async getAuthentication(): Promise<UUID_string> {
		if (OAKLEAN_AUTH_KEY !== undefined) {
			if (!Crypto.validateUniqueID(OAKLEAN_AUTH_KEY as UUID_string)) {
				throw new Error(
					'AuthenticationHelper.getAuthentication: Env Variable OAKLEAN_AUTH_KEY is no uuid4'
				)
			}
		...
		if (!fs.existsSync(authFile.toPlatformString())) {
			...
			PermissionHelper.changeFilePermission(
				authFile.toPlatformString(),
				PermissionTypes.ReadWriteOnlyOwner
			)
			return auth as unknown as UUID_string
		} else {
			const auth = fs
				.readFileSync(authFile.toPlatformString())
				.toString() as UUID_string
			if (!Crypto.validateUniqueID(auth)) {
				throw new Error(
					'AuthenticationHelper.getAuthentication: authentication string is no uuid4'
				)
			}
			return auth
		}
